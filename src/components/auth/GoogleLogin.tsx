...
export function GoogleLogin() {
  ...
  return (
    <button
      ...
      disabled={isLoading}
      className="w-full h-[58px] flex items-center justify-center gap-3 bg-[#1a0a03] rounded-full shadow-[0_4px_14px_rgba(0,0,0,0.15)] hover:bg-black transition-all active:scale-[0.98] mt-2 group border border-white/5"
    >
      {isLoading ? (
        <Loader2 size={24} className="animate-spin text-white/70" />
      ) : (
        <>
          <div className="bg-white p-1 rounded-full w-8 h-8 flex items-center justify-center shadow-inner">
          ...
          <span className="font-bold text-white/95 text-[16px]">Continue with Google</span>
        </>
      )}
    </button>
  );
