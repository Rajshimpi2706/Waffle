...
export function GoogleLogin() {
  ...
  return (
    <button
      ...
      disabled={isLoading}
      className="w-full h-16 flex items-center justify-center gap-4 bg-white border border-[#F5E6CC] rounded-[1.5rem] shadow-soft hover:shadow-medium hover:border-[#E5D5C0] transition-all active:scale-95 relative overflow-hidden group"
    >
      <div className="absolute inset-0 bg-gradient-to-r from-[#FDF6EC]/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
      
      {isLoading ? (
        <Loader2 size={24} className="animate-spin text-[#C17839]" />
      ) : (
        <>
          ...
          <span className="font-bold text-[#3B1F0A] text-lg lg:text-base">Continue with Google</span>
        </>
      )}
    </button>
  );
